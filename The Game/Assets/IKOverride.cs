...
public class IKOverride : StateMachineBehaviour
{
    ...
    public override void OnStateEnter(Animator animator, AnimatorStateInfo stateInfo, int layerIndex)
    {
        ...
        if(_solver)
            _originalGripWeight = _solver.GripWeight;
    }
    ...
    public override void OnStateUpdate(Animator animator, AnimatorStateInfo stateInfo, int layerIndex)
    {
        ...
        if (time < TweenBegin)
            _solver.GripWeight = Mathf.Lerp(_originalGripWeight, 0, time / TweenBegin);
        else if (time > TweenEnd)
            _solver.GripWeight = Mathf.Lerp(0, _originalGripWeight, (time - TweenEnd) / (1 - TweenEnd));
        else
            _solver.GripWeight = 0;
    }
    
    public override void OnStateExit(Animator animator, AnimatorStateInfo stateInfo, int layerIndex)
    {
        if(_solver)
            _solver.GripWeight = _originalGripWeight;
